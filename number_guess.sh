  ...
  #    then  
    ...
    do  
      ...
      then
        ...
        # update best_game
        UPDATE_BEST_GAME=$($PSQL "UPDATE users SET best_game=(SELECT number_of_guesses FROM games WHERE user_id = $USERID ORDER BY number_of_guesses ASC LIMIT 1) WHERE USER_id=$USERID;")
